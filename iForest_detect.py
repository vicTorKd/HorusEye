...
def transfer_rule(clf, path):
    INF = 50000
    from collections import defaultdict
    # record all the branches
    ...
    for e in clf.estimators_:
        ...
        for i in range(n_nodes):
            if (feature[i] < 0):  # leaf node
                continue
            branches[feature[i]].add(threshold[i])  # add the partition threshold of feature x
    for f in branches.keys():
        branches[f].add(INF)

    Feature_domain = pd.DataFrame({'key': 1}, index=[0])

    # Calculate the Table after the combination of the Cartesian product of each feature
    for f in branches.keys():
        temp = pd.DataFrame({f: tuple(branches[f]), 'key': 1})
        Feature_domain = pd.merge(Feature_domain, temp, on='key')

    # remove key used for cartesian product
    ...
    # assign a value to each feature field
    Feature_domain_label = clf.predict(Feature_domain)
    Feature_domain['label'] = Feature_domain_label
    Feature_domain.to_csv('./result/iForest/Feature_domain.csv')
    merge = pd.DataFrame()

    # record the left-open interval for each rule
    for f in branches.keys():
        Feature_domain[str(f) + '_left'] = Feature_domain[f]
    old_count = defaultdict(int)  # used to record left open interval
    ...
    for i in range(0, len(Feature_domain) - 1):
        # indicates that the label has not changed
        flag = False
        if (Feature_domain['label'].iloc[i] == Feature_domain['label'].iloc[i + 1]):
            for f in branches.keys():
                if Feature_domain[f].iloc[i + 1] == INF:  # to the border
                    merge[f].iloc[-1] = INF
            ...
            for f in branches.keys():
                ...
                # change the feature values that inducing difference
            merge = merge.append(temp)
    ...
    for i in range(len(merge) - 1):
        ...
        for f in branches.keys():
            if (merge[f].iloc[i] == merge[f].iloc[i + 1] and merge[str(f) + '_left'].iloc[i] ==
                ...
                for g in branches.keys():
                    if g != f:
                        if merge[str(g) + '_left'].iloc[i + 1] <= merge_[g].iloc[-1] and merge[g].iloc[i + 1] == \
                                merge_[g].iloc[-1] + 1:
                            flag = False
                            merge_[g].iloc[-1] = merge[g].iloc[i + 1]
    ...
    merge_.to_csv(path)
    # rule num
    print('rule num', len(merge_))


def train(device_name, feature_set, df_normal_train, df_attack_eval):
    df_normal_train, df_normal_eval = train_test_split(df_normal_train, test_size=0.5)
    ...
    for num_estimators in [200]:  # 100,200,300
        for m_samples in [5000]:  # 300,400
            for contamination in [0.25]:
                clf_udp = IsolationForest(n_estimators=num_estimators, max_samples=m_samples, random_state=123,
                                          contamination=contamination)
                clf_tcp = IsolationForest(n_estimators=num_estimators, max_samples=m_samples, random_state=123,
                                          contamination=contamination)
                ...
                print(
                    "n_estimators:{:},m_samples:{:},contamination:{:}".format(num_estimators, m_samples, contamination))
                print('Test')
                ...
                a = classification_report(y_true=eval_y, y_pred=y_pred_eval, target_names=['abnormal', 'normal'],
                                          output_dict=True)
                transfer_rule(clf_udp, './result/iForest/udp_rule_' + device_name + '.csv')
                transfer_rule(clf_tcp, './result/iForest/tcp_rule_' + device_name + '.csv')

                test_x = test(['all'], feature_set, df_eval)

                print('accuracy', accuracy_score(test_x['pred'], y_pred_eval))
                test_x[test_x['pred'] != y_pred_eval].to_csv('./result/iForest/wrong.csv')
                ...
                if (best_f1_score < temp_f1):
                    ...
                    eval_x.to_csv('./result/iForest/test_pred_' + device_name + '.csv')

    print("best n_estimators:{:},m_samples:{:}".format(best_n, best_m))
    print(best_str + '\n' + str(temp_f1))
    iforest_record.to_csv('./result/iForest/iforest_record.csv')
    return best_return
...
def test(device_list, feature_set, df_test):  #
    ...
    for device_name in device_list:
        udp_clf = udp_clf.append(pd.read_csv('./result/iForest/udp_rule_' + device_name + '.csv'))
        tcp_clf = tcp_clf.append(pd.read_csv('./result/iForest/tcp_rule_' + device_name + '.csv'))
    condition_udp = []
    condition_tcp = []
            ...
            if not len(condition1):
                condition1 = condition1 or (((udp_x_test[feature_set[f]] <= udp_clf[str(f)].iloc[i]) & (
                        udp_x_test[feature_set[f]] > udp_clf[str(f) + '_left'].iloc[i])) | (
                                                    udp_x_test[feature_set[f]] == udp_clf[str(f)].iloc[i]))
            else:
        ...
        for f in range(len(feature_set)):
            ...
            if not len(condition2):
                condition2 = condition2 or (((tcp_x_test[feature_set[f]] <= tcp_clf[str(f)].iloc[i]) & (
                        tcp_x_test[feature_set[f]] > tcp_clf[str(f) + '_left'].iloc[i])) | (
                                                    tcp_x_test[feature_set[f]] == tcp_clf[str(f)].iloc[i]))
            else:
    ...
    result_table = result_table.append(pd.DataFrame(
        {'device_name': device_name, 'abnormal_precision': round(return_table['abnormal']['precision'], 3),
         'abnormal_recall': round(return_table['abnormal']['recall'], 3),
         'normal_precision': round(return_table['normal']['precision'], 3),
         'normal_recall': round(return_table['normal']['recall'], 3), 'support': return_table['normal']['support'], },
        index=[0]))
    result_table.to_csv('./result/iForest/result_table.csv')
...
def get_Anomaly_ID_test(df_test_with_pred, df_test_data):  # DateFrame
    ...
    for thr in np.arange(0.05, 1.05, 0.1):
        ...
        temp_f1 = return_table['abnormal']['recall']
        if (best_f1_score < temp_f1):
            best_f1_score = temp_f1
            best_thr = thr
            best_return = return_table
    print("best threshold:{:}".format(best_thr))
    threshold_record.to_csv('./result/iForest/threshold_record_test.csv')
    return best_return
...
def main(attack_name='all', device_list=['philips_camera'], thr_time=10, file_num=5):
    ...
    anomaly_df.to_csv('./result/iForest/pre_anomaly_5-tuple.csv')

...
def roubust_process():
    ...
    normal_list = ['mix']
    for type_index, type_name in enumerate(normal_list):
        file_list = file_name_walk('../DataSets/{:}'.format(type_name))
        save_root = '../DataSets/robust/{}/attack-packet-level-device'.format(type_name)
        # file_list = file_name_walk('../DataSets/Normal/data/{:}'.format(type_name))
        # save_root = '../DataSets/normal-packet-level-device/{}'.format(type_name)
        if not os.path.exists(save_root):
            os.makedirs(save_root)
        ...
        for i, file_name in enumerate(file_list):
            if 'tcp' in file_name:
                ...
                continue
            print(file_name)
            save_path = save_root + '/{}.csv'.format(file_name.split('/')[-1].replace('.pcap',''))
            pcap2csv(file_name, save_path)

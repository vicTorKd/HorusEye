...
def data_processing(df):  # for testing
    scaler_path = './params/open-source/scaler.pkl' if OPEN_SOURCE else './params/our/scaler.pkl'
...
def train_data_processing_Kitsune(df_normal_train, df_attack_eval):
    # data preparing / transfer data format for Kitsune
    ...
    if OPEN_SOURCE:
        ...
        if not os.path.exists('./params/our/'):
            os.makedirs('./params/our/')
...
def train_autoencoder(model, df_normal_train, df_attack_eval, model_save_path):
    ...
    # init
    phi = 0  # maximum loss
    beta = 0.1  # beta*phi is anomaly threshold
    # record
    best_testing_acc = 0
    best_epoch = -1
...
def train_test_Kitsune(X_train, X_normal_eval, X_attack_eval, test_X, test_y):
    ...
    # loss_record.to_csv('./result/our/Kitsune_loss_record_test.csv')
...
if __name__ == "__main__":
    # Hyper parameters
    KITSUNE = False  # using kitsune model
    OPEN_SOURCE = False  # using open source data
    TRAIN = True
    TEST = True
    Use_filter = True  # using filter
    BATCH_SIZE = 128
    # TEST_BATCH_SIZE = 8000  # for int8 trt model
    TEST_BATCH_SIZE = 30000  # for fp32/fp16 trt model
    INPUTSIZE = 100

    # Pytorch model path
    model_save_path = './params/open-source/CNN_DW_dilation.pkl' if OPEN_SOURCE \
        else './params/our/CNN_DW_dilation.pkl'

    # TensorRT model path
    tensorrt_save_path = './params/our/tensorrt_fp32_CNN_DW_dilation.engine'

    # Kitsune model path
    Kitsune_save_path = './params/our/Kitsune_model.pkl'

    torch.cuda.set_device(0)
    device_list = ['philips_camera', '360_camera', 'ezviz_camera', 'hichip_battery_camera', 'mercury_wirecamera',
                   'skyworth_camera', 'tplink_camera',
                   'xiaomi_camera']

    # model
    model = CNN_AE(input_size=INPUTSIZE)
    # model = Kitsune(input_size=INPUTSIZE, hidden_rate=0.75)

    # feature in data plane
    feature_set = ['pk_num', 'sum_len']

    # load attack data in control plane
    df_attack = load_iot_attack_seq('all')

    # load attack data in data plane
    ...
    if TRAIN:
        ...
        if OPEN_SOURCE:
            ...
            df_normal_train_data = load_iot_data(device_list=device_list, thr_time=1, begin=0, end=4)
        print(df_normal_train_data.shape)
        iForest_detect.train('all', feature_set, df_normal_train_data, df_attack_data)

        # train AE
        df_attack_test, df_attack_eval = train_test_split(df_attack, test_size=0.2, random_state=20)
        if OPEN_SOURCE:
            df_normal_train = open_source_load_iot_data_seq(selected_list=[0, 2, 4, 6, 7])
        else:
            df_normal_train = load_iot_data_seq(device_list=device_list, begin=0, end=4)
    ...
    if TEST:  # for test
        ...
        if OPEN_SOURCE:
            ...
            df_normal_test_con = load_iot_data_seq(device_list=device_list, begin=4, end=6)
        df_test_con = df_normal_test_con.append(df_attack)  # the feature used in control plane

        # data plane
        if OPEN_SOURCE:
            df_normal_test_data = open_source_load_iot_data(thr_time=1, selected_list=[1, 3, 5, 8])
        else:
            df_normal_test_data = load_iot_data(device_list=device_list, thr_time=1, begin=4,
                                                end=6)  # the feature used in data plane
        df_test_data = df_normal_test_data.append(df_attack_data)
        ...
        # iForest_detect.get_Anomaly_ID_test(df_test_with_pred, df_test_data)  # test the iForest threshold
        anomaly_df = iForest_detect.get_Anomaly_ID(df_test_with_pred, 0.375)
        ...
        # transfer to test loader
        if not KITSUNE:
            test_datasets = Data.TensorDataset(test_X, test_y)
            test_loader = Data.DataLoader(dataset=test_datasets, batch_size=TEST_BATCH_SIZE, shuffle=True,
                                          num_workers=0)
        ...
        # test the model throughput
        test_throughput(model, 30000, test_X)
        ...
        # Calculate the auc of the overall framework
        fpr, tpr, thresholds = roc_curve(eval_y_label, rmse_list)
        auc_eval = auc(fpr, tpr)
        ...
        if OPEN_SOURCE:
            ...
            roc_record.to_csv('./result/our/roc_record.csv')
